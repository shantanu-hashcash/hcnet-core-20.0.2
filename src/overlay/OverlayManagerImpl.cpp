...
{
    hcnet::shuffle(peerList.begin(), peerList.end(), gRandomEngine);
}
                                        ...
                                        Peer::pointer peer)
{
    ...
    auto logMessage = [&](bool unique, std::string const& msgType) {
        CLOG_TRACE(Overlay, "recv: {} {} ({}) of size: {} from: {}",
                   ...
                   peer->msgSummary(hcnetMsg), msgType,
                   xdr::xdr_argpack_size(hcnetMsg),
                   mApp.getConfig().toShortString(peer->getPeerID()));
    };
    ...
    auto hash = shortHash::xdrComputeHash(hcnetMsg);
