                            ...
                            LedgerTxnEntry& entry, int64_t amount)
{
    ...
    if (!hcnet::addBalance(newBalance, amount, tl.limit))
    {
        return false;
    }
                              ...
                              Asset const& asset)
{
    ...
    for (auto& offer : offers)
    {
        ...
        auto trustAcc = hcnet::loadAccount(ltxInner, account);
                                   ...
                                   SequenceNumber txSeqNum, uint32_t opIndex)
{
    ...
    for (auto const& poolTLKey : poolTLKeys)
    {
        ...
        {
            auto trustAcc = hcnet::loadAccount(ltxInner, accountID);
        ...
        auto redeemIntoClaimableBalance =
            ...
            if (loadSponsorship(ltxInner, cbSponsoringAccID))
            {
                auto cbSponsoringLtxAcc =
                    hcnet::loadAccount(ltxInner, cbSponsoringAccID);
...
struct MuxChecker
{
    ...
    void
    operator()(hcnet::MuxedAccount const& t)
    {
