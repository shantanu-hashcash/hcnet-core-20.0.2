...
TEST_CASE("Trustline hcnet asset contract",
...
TEST_CASE("Native hcnet asset contract",
          "[tx][soroban][invariant][conservationoflumens]")
{
    auto cfg = getTestConfig();
...
TEST_CASE("Soroban footprint validation", "[tx][soroban]")
{
    ...
    // testValidRestoreOp
    auto invalidateFootprint = [&](xdr::xvector<hcnet::LedgerKey>& footprint,
                                   auto f) {
    ...
    SECTION("invokeHostFunction")
    {
        auto validateFootprint =
            [&](xdr::xvector<hcnet::LedgerKey>& footprint) {
                SECTION("valid")
                {
...
TEST_CASE("settings upgrade", "[tx][soroban][upgrades]")
{
    ...
    auto runTest = [&]() {
...

        {
            ...
            // https://github.com/hcnet/rs-soroban-env/blob/main/soroban-test-wasms/wasm-workspace/write_upgrade_bytes/src/lib.rs#L3-L5
...
TEST_CASE("contract storage", "[tx][soroban]")
{
    ...
    SECTION("footprint")
    {
        ...
        SECTION("unused readWrite key")
        {
            ...
            auto loadAccount = [&]() {
                LedgerTxn ltx(test.getApp().getLedgerTxnRoot());
                auto ltxe = hcnet::loadAccountWithoutRecord(ltx, acc);
                REQUIRE(ltxe);
                return ltxe.current();
            };
